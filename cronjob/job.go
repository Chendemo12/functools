...
// String 任务文字描述
func (c *Func) String() string { return "Func" }

// Interval 执行调度间隔
func (c *Func) Interval() time.Duration { return 5 * time.Second }

// Do 定时任务
func (c *Func) Do() func(ctx context.Context) error {
	fmt.Printf("%s Run at %s\n", c.String(), time.Now().String())
	return nil
}

// WhenTimeout 当任务超时时执行的回调
func (c *Func) WhenTimeout() {
...
//	scheduler := cronjob.NewScheduler(pCtx, logger)
//
//	// 定义任务, 需实现 CronJob 接口
//	type Clock struct {
//		cronjob.Func	// 默认 Func 未实现 Do 方法
//		lastTime time.Time
//	}
//
